    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 pb-20 relative">
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-lg flex items-center gap-2">
            Estimated Pipeline Value: <span className="text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-md border border-green-100 text-sm">€{stats?.revenue || 0}</span>
          </p>
        </div>
        <div className="flex gap-3">
           <button 
             onClick={handleAgendaCheckIn}
             className="bg-slate-900 text-white px-4 py-2 rounded-lg font-medium text-sm hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20 flex items-center gap-2"
           >
        ...
        <div className="col-span-4 bg-slate-50 rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col">
           ...
           <div className="mt-auto flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-600">Echo Dialer</span>
              </div>
              <button onClick={() => onNavigate('campaigns')} className="text-xs font-bold text-indigo-600 hover:underline">Open Dialer</button>
           </div>
        </div>
      ...
      <div className="grid grid-cols-12 gap-6">
         ...
         <div className="col-span-8 bg-white rounded-3xl border border-slate-200 shadow-sm p-1 min-h-[300px]">
            ...
            {isLoading ? (
                ...
                {recentActivity.filter((item: any) => new Date(item.id).toDateString() === new Date().toDateString()).map((signal: any) => (
                    <div key={signal.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50 transition-colors group cursor-pointer rounded-xl mx-1 my-1">
                    <div className={`p-2 rounded-xl bg-slate-50 border border-slate-100 group-hover:border-indigo-100 group-hover:bg-indigo-50 transition-colors`}>
                        {signal.type === 'call' ? <Zap className="w-3.5 h-3.5 text-amber-500" /> : 
                         signal.type === 'email' ? <MessageSquare className="w-3.5 h-3.5 text-blue-500" /> :
                    ...
                    <button className="opacity-0 group-hover:opacity-100 transition-opacity bg-white border border-slate-200 shadow-sm px-4 py-1.5 rounded-lg text-xs font-bold text-slate-600 hover:text-indigo-600 hover:border-indigo-200">
                        {signal.action}
                    </button>
                    </div>
                ))}
         ...
         <div className="col-span-4 bg-slate-900 rounded-3xl p-6 text-white relative overflow-hidden border border-slate-800">
            ...
            <h3 className="text-sm font-bold text-purple-300 uppercase tracking-wider mb-4">Daily Coach</h3>
