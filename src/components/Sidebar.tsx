...
export function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  const navItems = [
    ...
    { id: 'live-campaigns', label: 'Live Campaigns', icon: PhoneCall, badge: 12 },
    ...
    <div className="w-64 bg-[#0F172A] text-slate-300 flex flex-col h-screen border-r border-slate-800">
      ...
      <div className="mx-4 mb-6 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Session 2</span>
          <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div>
        </div>
        <div className="text-3xl font-bold text-white mb-1">0</div>
        <div className="text-xs text-slate-500 mb-3">calls done</div>
      ...
      <div className="p-4 border-t border-slate-800">
        ...
        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800/50 transition-colors cursor-pointer">
          ...
          <div className="flex-1 overflow-hidden">
            <div className="text-sm font-medium text-white truncate">Pepa Trader</div>
