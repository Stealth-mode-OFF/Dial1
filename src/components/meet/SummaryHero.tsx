...
export function SummaryHero({
  ...
  useEffect(() => {
    ...
    echoApi.emailSchedule.active({ contactId: lead.id })
      .then((res) => {
        if (cancelled) return;
        const rows = (Array.isArray(res?.schedules) ? res.schedules : []) as ScheduleRow[];
        const has = rows.some((row) => String(row?.email_type || '').startsWith('sequence-') && (row?.status === 'pending' || row?.status === 'draft-created'));
    ...
    <div className="mc-summary">
      ...
      <div className="mc-summary-stats">
        ...
        {SPIN_PHASES.map((phase) => (
          <div key={phase.id} className="mc-summary-stat" style={{ '--phase-color': phase.color } as CSSProperties}>
      ...
      <div className="mc-ai-wrapup">
            ...
            <div className="mc-ai-wrapup-body">
              ...
              {Array.isArray(analysis.strengths) && analysis.strengths.length ? (
                <div className="mc-ai-list">
                  ...
                  <ul>{analysis.strengths.slice(0, 4).map((s, i) => <li key={`${s}-${i}`}>{s}</li>)}</ul>
                </div>
              ) : null}
              {Array.isArray(analysis.weaknesses) && analysis.weaknesses.length ? (
                <div className="mc-ai-list">
                  <div className="mc-ai-list-title">Slabiny</div>
                  <ul>{analysis.weaknesses.slice(0, 4).map((s, i) => <li key={`${s}-${i}`}>{s}</li>)}</ul>
                </div>
              ) : null}
                  ...
                  <div className="mc-ai-email-hint muted">
                    ...
                    <ul style={{ margin: '6px 0 0 16px' }}>
                      {emailHistory.slice(0, 3).map((item) => {
                        const when = item?.sent_at ? new Date(String(item.sent_at)).toLocaleDateString('cs-CZ') : '—';
                        ...
                        return <li key={String(item?.id || `${when}-${type}-${subject}`)}>{when} · {typeLabel} · {subject}</li>;
                      })}
                    </ul>
                  </div>
                ) : (
        ...
        <div className="mc-ai-email" style={{ marginTop: 12 }}>
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  const d1 = computeSequenceIso(1, sequenceTime, sequenceTimeZone);
                  const d3 = computeSequenceIso(3, sequenceTime, sequenceTimeZone);
                  const baseContext = {
                    sequenceKind: 'demo',
                    contactName: lead.name,
                    company: lead.company,
          ...
          <div className="mc-ai-email-hint muted">→ D+1: krátký bump ({formatSequenceWhen(computeSequenceIso(1, sequenceTime, sequenceTimeZone), sequenceTimeZone)})</div>
          <div className="mc-ai-email-hint muted">→ D+3: finální follow‑up ({formatSequenceWhen(computeSequenceIso(3, sequenceTime, sequenceTimeZone), sequenceTimeZone)})</div>
